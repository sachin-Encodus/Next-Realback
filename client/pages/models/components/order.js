...
const Order = ({ adds, cartdata, price, device, image }) => {
  const notify = React.useCallback((type, message) => {
    toast({ type, message });
  }, []);
  const router = useRouter();
  const cartarr = useSelector((state) => state.cartdata.cartData);

  console.log("=======xxxxxxxxxxxx", cartarr);
  const status = "orderd";
    ...
    if (
      ...
      axios
        ...
        .then((res) => {
          ...
          notify("success", `Order Otp has been sent to ${email}`);
                  ...
                  <div className={style}>
                    ...
                    <div class="agreement">
                      <div class="realback-logo">
                        <img src="/images/realback.png" alt="7" />
                      </div>
...
export async function getServerSideProps(ctx) {
  const { cartdata, price, device, image } = ctx.query;
