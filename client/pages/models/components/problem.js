...
const Problem = ({ probdata, dname, probId }) => {
              ...
              <>
                <div className="col-md-12 glass">
                  ...
                  <Link
                    style={{ padding: 18 }}
                    href={{
                      pathname: "/models/components/order",
                      query: {
                        cartdata,
                        price: totalPrice,
                        device: dname,
...
export async function getServerSideProps(ctx) {
  const { id, probId, dname } = ctx.query;

  const res = await axios.get(`/api/getmodedata/${id}`, {
  ...
  return {
    props: {
      probId,
      dname,
      probdata: res.data.device.model,
    },
  };
}
